...
export class DashboardComponentComponent implements OnInit {
  ...
  activeChartTab: 'Revenue' | 'Users' | 'Conversion' = 'Revenue';
  
  dateRanges: string[] = ['Last 7 Days', 'Last 30 Days', 'Last 90 Days', 'This Year'];
  ...
  // Switch Chart Tabs
  setChartTab(tab: 'Revenue' | 'Users' | 'Conversion'): void {
  ...
  exportReport(): void {
    const csvContent = "data:text/csv;charset=utf-8," 
      ...
      + this.recentTransactions.map(t => `"${t.name}","${t.email}","${t.plan}","${t.amount}","${t.status}","${t.date}"`).join("\n");
  ...
  viewAllTransactions(): void {
    alert('Redirecting to full transactions directory table...');
  }

  // Row Action Handler
  onTransactionAction(tx: Transaction): void {
    alert(`Opening action menu for transaction: ${tx.name} (${tx.amount})`);
  }
}